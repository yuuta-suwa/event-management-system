...
export function EventForm({action,categories,event}:Props){
  ...
  if(state!==syncedState){setSyncedState(state);setSelectKey(k=>k+1)}
  useEffect(()=>{
    if(!isNew)return;
  ...
  <section className="form-section"><div><span>01</span><h2>基本情報</h2></div><div className="form-grid">
    ...
    <label>カテゴリー<select key={`category-${selectKey}`} name="categoryId" defaultValue={v("categoryId",event?.categoryId??"")} required><option value="" disabled>選択してください</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>{err("categoryId")&&<small>{err("categoryId")}</small>}</label>
    <label>公開状態<select key={`status-${selectKey}`} name="status" defaultValue={v("status",event?.status??"DRAFT")}><option value="DRAFT">非公開（下書き）</option><option value="PUBLISHED">公開・受付中</option><option value="CLOSED">受付終了</option></select></label>
