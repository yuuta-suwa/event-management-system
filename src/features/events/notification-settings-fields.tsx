...
export function NotificationSettingsFields({defaults,restoreDraft}:Props){
  ...
  return <div ref={containerRef} className="form-grid">
    ...
    <label>前日通知の時刻<input type="time" name="eveNotificationTime" value={eveTime} onChange={e=>setEveTime(e.target.value)}/></label>
    ...
    <label>当日通知の時刻<input type="time" name="dayOfNotificationTime" value={dayTime} onChange={e=>setDayTime(e.target.value)}/></label>
    ...
    <label>開始何分前<select name="beforeStartNotificationMinutes" value={beforeMin} onChange={e=>setBeforeMin(Number(e.target.value))}><option value={180}>3時間前</option><option value={60}>1時間前</option><option value={30}>30分前</option></select></label>
